...
public class GamePageParser {
	...
	//Exercise 9-30. Save each box score to a  le
	private void saveResultList(List<Result> results) {
		results.parallelStream()
			   .forEach(this::saveResultToFile);
	}
	...
	//Exercise 9-31. Getting the maximum total score and the game where it occurred
	...
	//Exercise 9-32. Main application code
	public void printGames(LocalDate startDate, int days) {
		...
		CompletableFuture<String> futureMax = futureMaxScore.thenCombineAsync(futureMaxGame, 
												(score,result) -> String.format("Highest score: %d, Max Game: %s",
